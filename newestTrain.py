...
lora_config = LoraConfig(
    ...
    target_modules=["c_attn", "c_proj", "fc_in", "fc_out"] # Typical for LLaMA
)
...
def train(model, dataloader, valloader, optimizer, device, epochs=3):
    ...
    for epoch in range(epochs):
        ...
        for img, ids, mask in pbar:
            ...
            if counter%1000 == 0:
                ...
                out_epoch_loss = train_epoch_loss/(counter*16)
        ...
        val_epoch_loss = 0
        with torch.no_grad():    
            pbar = tqdm(valloader, desc=f"Epoch {epoch+1}")
            for img, ids, mask in pbar:
                img = img.to(device)
                ids = ids.to(device)
                mask = mask.to(device)
        ...
        epoch_losses.append(val_epoch_loss)

        logits = outputs.logits[:,576:, :]
        pred_ids = torch.argmax(logits, dim=-1)
        out(epoch,epochs,val_epoch_loss,caps_out,mask,pred_ids)
        ...
        if val_epoch_loss < best_val_loss:
            ...
            print(f"✅ Best model updated with val_loss={val_epoch_loss:.4f}")

os.makedirs("checkpoints", exist_ok=True)
        
training = flickr_dataset(train_set)
val = flickr_dataset(val_set)

train_loader = DataLoader(training, batch_size=16, shuffle=True, num_workers=8, pin_memory=True)
val_loader = DataLoader(val, batch_size=16, shuffle=True, num_workers=8, pin_memory=True)
