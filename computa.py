...
def train(model, dataloader, optimizer, device, epochs=3):
    ...
    for epoch in range(epochs):
        ...
        for img, caption in pbar:
            ...
            caps = (cap['input_ids'][:,0,:]).to(device)

            mask = cap['attention_mask'][:,0,:].to(device)
            ...
            if counter%100 == 0:
                ...
                out(epoch,epochs,out_epoch_loss,caps,mask,pred_ids)
            counter += 1

        out_epoch_loss = train_epoch_loss/len(train_loader)
        epoch_losses.append(out_epoch_loss)
...
train_loader = DataLoader(val, batch_size=16, shuffle=True, num_workers=8, pin_memory=True)
